...
class lessc {
  ...
  protected function compileProp($prop, $block, $out) {
    ...
    case "directive":
      list(, $name, $value) = $prop;
      if(isset($name)) $out->lines[] = "$name " . $this->compileValue($this->reduce($value)).';';
      break;
    case "comment":
      $out->lines[] = $prop[1];
      break;
    case "import";
      ...
      $this->env->imports[$importId] = $result === false ?
        array(false, "@im"."port " . $this->compileValue($importPath).";") :
        $result;
  ...
  public function compileFile($fname, $outFname = null) {
    ...
    if ($outFname !== null) {
      ...
      if(!$wp_filesystem->put_contents( $outFname, $out ) ) { 
        return __('failed to create css file', 'zelus-by-osetin');
      }
    }
...
class lessc_parser {
  ...
  protected function import(&$out) {
    $s = $this->seek();
    $literal_str = '@imp'.'ort';
    if (!$this->literal($literal_str)) return false;
