...
function osetin_autosuggest_process_request(){
  if(isset($_POST['search_query_string'])){
      ...
      while ( $osetin_recipes_query->have_posts() ) : $osetin_recipes_query->the_post();
        $response_html.= '<a href="'.get_the_permalink().'" class="autosuggest-item">';
          $response_html.= '<div class="autosuggest-item-media-w">';
            $response_html.= '<div class="autosuggest-item-media-thumbnail fader-activator" style="background-image:url('.osetin_output_post_thumbnail_url("large", false).'); background-size: cover;">';
    ...
    if($response_html){
      ...
      echo wp_send_json(array('status' => 404, 'message' => esc_html__('No recipes found', 'zelus-by-osetin')));
    }

  }else{
    echo wp_send_json(array('status' => 422, 'message' => esc_html__('Invalid data supplied', 'zelus-by-osetin')));
  }
}
